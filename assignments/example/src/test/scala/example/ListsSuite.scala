...
class ListsSuite extends FunSuite {
  ...
  test("Must throws an exception when the list is empty") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }

  test("Must find the max for negative numbers") {
    assert(max(List(-10, -2, -3, -1)) === -1)
  }

  test("find the max when there is just one number") {
    assert(max(List(9)) === 9)
  }
}
